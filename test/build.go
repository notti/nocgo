...
func (t Test) FFI() string {
	...
	if !t.Ret.Void() {
		args = append(args, fmt.Sprint("\tret ", t.Ret.Go(), " `ffi:\"ret\"`"))
	}
	return strings.Join(args, "\n")
}
...
var testingFFI = template.Must(template.New("testingFFI").Parse(`package testlib
...
func main() {
	...
	testingFFIfile, err := os.Create("ffi/ffi_test.go")
	if err != nil {
		log.Fatalln("Couldn't open bridge file")
	}
	tests := []Test{
	...
	if err := testingFFI.Execute(testingFFIfile, tests); err != nil {
		log.Fatal(err)
	}
	testingFFIfile.Close()
}
