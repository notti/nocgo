...
func doCopy() {
	src, err := os.Open(in)
	if err != nil {
		log.Fatal("couldn't open", in, ":", err)
	}
	defer src.Close()
	dst, err := os.OpenFile(out, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0777)
	if err != nil {
		log.Fatal("couldn't create", out, ":", err)
	}
	defer dst.Close()
	_, err = io.Copy(dst, src)
	if err != nil {
		log.Fatal("couldn't copy:", err)
	}
}
...
func main() {
	...
	*/
	...
	for _, sym := range symbolList {
		if dynsym, ok := relocations[sym.Name]; ok {
